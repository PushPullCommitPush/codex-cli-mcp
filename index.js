...
const FALLBACK_MODELS = [
  ...
  { id: 'security', name: 'Kimi K2 Thinking', base_model_id: 'kimi-k2-thinking:cloud' } // kept for alias mapping; filtered later
];
...
function renderSecurityConfig(securityProfile) {
  return '# Security profile disabled\n';
}
...
async function loadProfilesAndSyncConfigs() {
  ...
  // security profile disabled; no secondary config
...
function buildEnvForProfile(profileId, isSecurity) {
  ...
  env.CODEX_QUIET = '1';
  env.CODEX_HOME = MAIN_CODEX_HOME;
  return env;
}
...
async function resumeCodex(prompt, profile) {
  ...
  return new Promise((resolve) => {
    ...
    const cmd = ['--dangerously-bypass-approvals-and-sandbox'];
    if (profileId && profileId !== 'default') {
